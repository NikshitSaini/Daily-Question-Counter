...
function addEntry() {
    ...
    if (!questionCount || questionCount <= 0 || !username) return;
    ...
    fetch('http://localhost:3002/store-data', {
        ...
        body: JSON.stringify({ username, date: dateString, count: questionCount })
    });
}
...
function saveData() {
    ...
    data.forEach(entry => {
        fetch('http://localhost:3002/store-data', {
            ...
            body: JSON.stringify({ username, date: entry.date, count: entry.count })
        });
    });

    // Send notes data to the server
    Object.keys(notesData).forEach(date => {
        fetch('http://localhost:3002/store-notes', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ username, date, notes: notesData[date] })
        });
    });
...
async function syncData() {
  ...
  const serverQuestionData = await response.json();
  localStorage.setItem('questionData', JSON.stringify(serverQuestionData));

  const notesResponse = await fetch('http://localhost:3002/get-notes', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
  ...
  const serverNotesData = await notesResponse.json();
  localStorage.setItem('notesData', JSON.stringify(serverNotesData));
...
async function saveNotes() {
    ...
    // Delete previous data of the username
    await fetch('http://localhost:3002/delete-data', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    // Save current notes data to the server
    await fetch('http://localhost:3002/store-notes', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ username, date, notes: notesText })
    });
...
// Add event listener to sync data button
...
async function syncData() {
    ...
    for (const entry of questionData) {
        await fetch('http://localhost:3002/store-data', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    // Send notes data to the server
    for (const [date, notes] of Object.entries(notesData)) {
        await fetch('http://localhost:3002/store-notes', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ username, date, notes })
        });
    }
